...
{
   cancelAll(%this);
   if (%this.getDataBlock().className $= "Gem") {
      echo("Gem" SPC %this SPC "is respawning");
      if (!isObject(%this.pickup) || (%this.pickup.respawning)) {
         %this.hide(false);
         echo("Now visible, pickup is \"" @ %this.pickup @ "\"");
         %this.pickup = "";
      }
