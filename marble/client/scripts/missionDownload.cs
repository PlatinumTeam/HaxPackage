...
function onMissionDownloadPhase1(%missionName, %musicTrack)
{
   ...
   $LoadingGui::LastFile = false;
   LoadingProgress2.setVisible(false);
   LOAD_FileName.setVisible(false);
...
function onFileChunkReceived(%file, %progress, %total) {
//   echo(%a0 SPC %a1 SPC %a2);
   if ($LoadingGui::LastFile !$= %file) {
      $LoadingGui::LastFile = %file;
      LOAD_FileName.setText("<font:Arial:18><just:center>" @ %file);
      LOAD_FileName.setVisible(true);
   }
   LoadingProgress2.setVisible(true);
   LoadingProgress2.setValue(%progress / %total);
   Canvas.repaint();
}