...
function escapeFromGame()
{
   ...
   if ( $Server::ServerType $= "SinglePlayer" )  {
      ...
      PauseAdminBox.setVisible($HostingMP || $Server::StandingHost);
      PauseBox.setVisible(!($HostingMP || $Server::StandingHost));
      PauseText.setText("<just:center><font:DomCasualD:36>Paused");
      PauseAdminText.setText("<just:center><font:DomCasualD:36>Paused");
      Canvas.pushDialog(PauseDlg);
   }
}
...
//Matt
moveMap.bind(keyboard, "return", showChat);
...
function mouseFire(%val)
{
   $mvTriggerCount0++;
   $mouseFire = !$mouseFire;
   commandToServer('MouseFire', $mouseFire);
}
